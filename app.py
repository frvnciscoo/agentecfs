...
def procesar_celulosa(rutas):
    ...
    try:
        # --- AUXILIAR DE NORMALIZACIÓN DE CÓDIGOS ---
        def norm_cod(serie):
            return (
                serie.astype(str)
                .str.strip()
                .str.replace(r'\.0$', '', regex=True)
                .str.replace(r'\s+', '', regex=True)  # Elimina espacios internos invisibles
                .str.upper()
            )

        # 1. Cargar PROGRAMA usando la limpieza robusta
        programa = leer_y_limpiar_excel(rutas['programa'])
        programa['Entrega'] = norm_cod(programa['Entrega'])
        
        # 2. Cargar TOOLS (soporta múltiples archivos) con limpieza robusta
        ...
        # Normalizar columnas llave en Tools si existen
        if "Contrato" in tools.columns:
            tools["Contrato"] = norm_cod(tools["Contrato"])
        if "Orden_Pedido" in tools.columns:
            tools["Orden_Pedido"] = norm_cod(tools["Orden_Pedido"])

        # 3. Cargar SALDOS
        if 'saldos' in rutas and rutas['saldos']:
            try:
                saldos = leer_y_limpiar_excel(rutas['saldos'])
                saldos['Entrega'] = norm_cod(saldos['Entrega'])
            except:
                saldos = pd.DataFrame(columns=["Entrega", "Box Saldo"])
        else:
            saldos = pd.DataFrame(columns=["Entrega", "Box Saldo"])

        saldos['Box Saldo'] = pd.to_numeric(saldos['Box Saldo'], errors='coerce').fillna(0)
        
        # 4. Filtrar Históricos
        if 'historico' in rutas and rutas['historico']:
            ...
            if excluidas:
                ...
                # Asegurarse de que las pestañas excluidas estén normalizadas
                excluidas_norm = {str(x).strip().replace('.0', '').upper() for x in excluidas}
                programa = programa[~programa['Entrega'].isin(excluidas_norm)].copy()
        ...
        # 5. Filtrar Programa solo para Celulosa válida
        prog_filtrado = programa[
            (~programa["Entrega"].isin(entregas_con_saldo)) & 
        ...
        if not prog_cb.empty:
            if "Contrato" in tools.columns and "Expedicion" in tools.columns:
                ...
                tools_cb = tools[tools["Contrato"].isin(entregas_validas_cb)].copy()
        ...
        if not prog_dp.empty:
            if "Orden_Pedido" in tools.columns and "Cnt_Sigla" in tools.columns:
                ...
                tools_dp = tools[tools["Orden_Pedido"].isin(entregas_validas_dp)].copy()
